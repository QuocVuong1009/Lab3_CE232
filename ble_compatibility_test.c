...
static uint8_t raw_adv_data[] = {
        ...
        0x03, 0x03, 0xFF, 0x00,
        /* device name */
        0x0E, 0x09, 'L', 'O', 'P', '2', 'N', 'H', 'O', 'M', '3', 'L', 'A', 'B', '3'
};
...
static const uint8_t char_value[8] = {0x62, 0x61, 0x55, 0x47, 0x62, 0x61, 0x55, 0x47};
        ...
        case ESP_GATTS_WRITE_EVT:
            if (!param->write.is_prep){
                ...
                if(gatt_db_handle_table[IDX_CHAR_VAL_A] == param->write.handle){ 
                        ...
                        page = page + 1;
                        if (param->write.value == 0x00)
                        {
                            vTaskDelete(NULL);
                        }
                        task_ssd1306_display_text(param->write.value, page);
                }
...
void app_main(void)
{
    ...
    ssd1306_init();
    //task_ssd1306_display_clear(long_write);
    //esp_ble_gatts_cb_param_t test;

    //char *text = test->write.value;
   /* while (1)
    {
        char text = (char) char_value;
        if (strcmp(text, compare))
        {
